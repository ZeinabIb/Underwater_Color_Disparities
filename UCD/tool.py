...
def ACES(img, k):
    ...
    d = 0.59
    e = 0.14
    img[:, :, 0] = (img[:, :, 0] * (a * img[:, :, 0] + b)) / (
        img[:, :, 0] * (c * img[:, :, 0] + d) + e
    )
    img[:, :, 1] = (img[:, :, 1] * (a * img[:, :, 1] + b)) / (
        img[:, :, 1] * (c * img[:, :, 1] + d) + e
    )
    img[:, :, 2] = (img[:, :, 2] * (a * img[:, :, 2] + b)) / (
        img[:, :, 2] * (c * img[:, :, 2] + d) + e
    )
    return img
...
def Rescale_intensity(image_rgb, x):
    ...
    image_rgb[:, :, 0] = exposure.rescale_intensity(
        image_rgb[:, :, 0], in_range=(p2, p98)
    )
    p2, p98 = np.percentile(image_rgb[:, :, 1], (x, 100 - x))
    image_rgb[:, :, 1] = exposure.rescale_intensity(
        image_rgb[:, :, 1], in_range=(p2, p98)
    )
    p2, p98 = np.percentile(image_rgb[:, :, 2], (x, 100 - x))
    image_rgb[:, :, 2] = exposure.rescale_intensity(
        image_rgb[:, :, 2], in_range=(p2, p98)
    )
    return image_rgb
...
def color_balance(img, a, b, c):
    midtones_coef = [a, b, c]
    mid_coef_self = np.array([[-1, 1, 1], [1, -1, 1], [1, 1, -1]])
    mid_coef_self1 = np.array([1, 1, 1])
    tmp = np.sort(midtones_coef)
    midtones_coef = midtones_coef - tmp[1]
    midtones_gamma = np.exp(
        np.dot(mid_coef_self1, np.dot(np.diag(1 * midtones_coef), mid_coef_self))
    )
    for i in range(3):
...
def High_pass(img, sigma):
    gauss_out = gaussian(img, sigma=sigma)
...
def newbluegreen(i1, a, k):
    ...
    while gb < ab:
        if k == 0:
            ...
            i1[:, :, 2] = (
                i1[:, :, 2]
                + (n == 0) * a * (1 - i1[:, :, 2]) * (g_ - b_) * i1[:, :, 1]
                + (n != 0) * (a + ab - gb) * (1 - i1[:, :, 2]) * (g_ - b_) * i1[:, :, 1]
            )
        else:
            i1[:, :, 2] = scale(i1[:, :, 2])
            i1[:, :, 1] = (
                i1[:, :, 1]
                + (n == 0) * a * (1 - i1[:, :, 1]) * (b_ - g_) * i1[:, :, 2]
                + (n != 0) * (a + ab - gb) * (1 - i1[:, :, 1]) * (b_ - g_) * i1[:, :, 2]
            )
        n += 1
...
def newred(i1, a):
    ...
    while rg < ab:
        ...
        i1[:, :, 0] = (
            i1[:, :, 0]
            + (n == 0) * a * (1 - i1[:, :, 0]) * (g_ - r_) * i1[:, :, 1]
            + (n != 0) * (a + ab - rg) * (1 - i1[:, :, 0]) * (g_ - r_) * i1[:, :, 1]
        )
        n += 1
